...
const Companies = () => {
  ...
  const location = useLocation();  // <-- to access current URL params
  ...
  const fetchCompanies = async () => {
    try {
      ...
      // Build query string based on page and statusFilter
      ...
      const response = await axios.get(`/admin/get-contractors?${queryParams.toString()}`);
  ...
  useEffect(() => {
    // Fetch companies when the page or status filter changes
    fetchCompanies();
  }, [page, statusFilter]);
  ...
  return (
    <div className="p-6 min-h-screen pt-2">
      {/* Title */}
  <div className="background-gradients relative p-6 rounded-xl flex justify-between text-white shadow-md border border-gray-700 overflow-hidden mt-4">
 ...
 <div className="flex mt-4">
  {/* "All" button – no rounded corners */}
  <button
    onClick={() => handleStatusChange("all")}
    className={`px-6 py-2 text-sm font-semibold transition-all flex items-center justify-center ${
      statusFilter === "all"
