...
import { FaChevronLeft, FaChevronRight, FaUser, FaFileAlt } from "react-icons/fa";
        ...
        <div className="w-full rounded-xl overflow-x-auto space-y-4">
          ...
          {reports.length === 0 ? (
            ...
            reports.map((report, idx) => (
              <div
                key={report.id}
                className="grid grid-cols-8 gap-4 items-center text-sm text-gray-200 border border-gray-700 bg-gray-800 bg-opacity-40 p-4 rounded-lg hover:bg-opacity-60 transition"
              >
                ...
                <div className="font-semibold truncate" title={report.name}>
                  {/* Wrap the name in a Link to navigate to the report details page */}
                  <Link to={`/app/report-details/${report.id}`} className="text-blue-400 hover:underline">
                    {report.name}
                  </Link>
                </div>
